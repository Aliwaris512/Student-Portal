...
@router.get('/attendance/{student_id}/{st_course}')
...
def see_tasks(student_id:int, db:Session=Depends(get_db),
    ...
    # Join Tasks with Enrollment by course_id
    enrolled_courses = db.query(model.Enrollment.course_id).filter(model.Enrollment.student_id == student_id).subquery()
    get_tasks = db.query(model.Tasks).filter(model.Tasks.course_id.in_(enrolled_courses)).all()
    if not get_tasks:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    return {"Tasks": [
        {
            ...
            "upload_date": str(task.upload_date),
            "due_date": str(task.due_date),
            "course_id": task.course_id
        } for task in get_tasks
    ]}    

@router.post('/upload/tasks/{student_id}/{task_id}')
